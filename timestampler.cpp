...
TimeStampler::TimeStampler(QObject *parent): m_deviceHandler(0)
{
    ...
    m_timeout_timer.setInterval(25);
}
...
uint32_t TimeStampler::get_diff_in_us_to_current_ts(uint32_t someTimeStamp)
{
    return ( ( m_etimer.nsecsElapsed() / 1000 ) - someTimeStamp );
}
...
void TimeStampler::send_timer_expired()
{
    ...
    {
        // start or some other not interesting state))
    }
}

void TimeStampler::timeout_timer_expired()
{
    qCritical()<<"TimeSync Failed - > Timeout!";
    m_sync_state = UNUSED;
}
